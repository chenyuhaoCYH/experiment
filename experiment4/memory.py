...
class ExperienceBuffer:
    ...
    def sample(self, batch_size):
        ...
        cur_otherState, cur_NeighborState, all_vehicle_state, \
        bandAction, aimAction, freqAction, rewards, \
        next_otherState, next_NeighborState, next_all_vehicle_state = zip(*[self.buffer[idx] for idx in indices])
        # 转换成numpy
        return np.array(cur_otherState), np.array(cur_NeighborState), np.array(all_vehicle_state), \
               np.array(bandAction), np.array(aimAction), np.array(freqAction), \
               np.array(rewards, dtype=np.float32), \
               np.array(next_otherState), np.array(next_NeighborState), np.array(next_all_vehicle_state)
